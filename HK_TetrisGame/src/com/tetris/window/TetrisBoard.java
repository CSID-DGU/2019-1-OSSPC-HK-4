...
public class TetrisBoard extends JPanel implements Runnable, KeyListener, MouseListener, ActionListener{
	...
	protected void paintComponent(Graphics g) {
		...
		// ip, port, nickname의 글씨 색 때문에 이곳에 쓴 것 같다 (캔버스의 색을 하얗게 칠한다)
		// 좌우 네모박스의 크기를 좀 조절하면 되지 않나? -> newFont와 setColor 추가함으로써 해결, 좌우 네모박스 크기도 수정했다. 
		...
		g.drawString("닉네임 : "+nickName, 20, 40);
			// 2P ip, nickName 출력
				// IP 출력
				g.drawString("ip : "+ip, 2*BOARD_X+maxX*BLOCK_SIZE+20, 20);
				//NickName 출력
				g.drawString("닉네임 : "+nickName, 2*BOARD_X+maxX*BLOCK_SIZE+20, 40);
		// 속도 출력
